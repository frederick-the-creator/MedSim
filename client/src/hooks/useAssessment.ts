...
export function useAssessment(): UseAssessmentResult {
	...
	const run = useCallback(
		async (conversationId: string, medicalCase: MedicalCase) => {
			setLoading(true);
			setError(null);
			try {
				...
				setAssessment((json.assessment as Assessment) ?? null);
				setTranscript((json.transcript as string) ?? null);
			} catch (e: any) {
...
export function useAssessmentMock(): UseAssessmentResult {
	...
	const run = useCallback(
		async (_conversationId: string, _medicalCase: MedicalCase) => {
			setLoading(true);
			setError(null);
			try {
