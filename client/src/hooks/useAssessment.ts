...
export type UseAssessmentResult = {
	...
	) => Promise<{ assessment: Assessment | null; transcript: string | null }>;
	...
	const runAssessment = useCallback(
		async (
			...
			try {
				...
				if (!resp.ok) {
					const errJson = (await resp.json().catch(() => ({}) as any)) as any;
					const message =
						(errJson && (errJson.message as string)) ||
						`Server assessmentError ${resp.status}`;
					throw new Error(message);
				}
				const json = await resp.json();
				const nextAssessment = (json.assessment as Assessment) ?? null;
				const nextTranscript = (json.transcript as string) ?? null;
				setAssessment(nextAssessment);
				setTranscript(nextTranscript);
				return { assessment: nextAssessment, transcript: nextTranscript };
			} catch (e: any) {
...
export function useAssessmentMock(): UseAssessmentResult {
	const [assessment, setAssessment] = useState<Assessment | null>(null);
	const [transcript, setTranscript] = useState<string | null>(null);
	const [assessmentLoading, setAssessmentLoading] = useState(false);
	const [assessmentError, setAssessmentError] = useState<string | null>(null);
