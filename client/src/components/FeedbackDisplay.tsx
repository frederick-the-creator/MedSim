...
export default function FeedbackDisplay({ feedback, onNewCase }: FeedbackDisplayProps) {
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6">
      <div>
        <h2 className="text-4xl font-bold mb-4" data-testid="text-feedback-title">
          Consultation Feedback
        </h2>
        <Card className="bg-accent/30 border-accent">
          <CardContent className="pt-6">
            <p className="text-base leading-relaxed" data-testid="text-overall-feedback">
              {feedback.overall}
            </p>
          </CardContent>
        </Card>
      ...
      <div>
        ...
        <Accordion type="multiple" defaultValue={feedback.sections.map((_, idx) => `item-${idx}`)} className="space-y-4">
      ...
      <div className="flex gap-4 pt-6 border-t">
        <Button onClick={onNewCase} className="flex-1" data-testid="button-new-case">
          Try Another Case
        </Button>
      </div>
    </div>
